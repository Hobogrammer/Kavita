...
test.describe('Login functionality', () => {
  ...
  test('redirects to `/home` if successful', async ({page}) => {
    ...
    // Set routes required to load loging page
    await setRoute(page, environment.apiUrl + 'admin/exists', true);
    await setRoute(page, environment.apiUrl + 'theme', [siteTheme]);
    await setRoute(page, environment.apiUrl + 'locale', localeResponse);
    ...
    // Assert
    await expect(page.url()).toBe('http://localhost:4200/home');
  });

/*
  test('redirected to previous url if was redirected to login and successful', async ({page}) => {

  });

  test('errors if unsuccessful', async ({page}) => {
    await setRoute(page, environment.apiUrl + 'account/login', {}, {status: 401});
  });
*/
});
